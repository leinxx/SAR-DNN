...
DataTransformer<Dtype>::DataTransformer(const TransformationParameter& param)
  ...
  // check channel_wise_Mean_file
  if (param_.has_mean_value_file())  {
    CHECK_EQ(param_.mean_value_size(), 0);
    CHECK_EQ(param_.has_mean_file(), 0);

    std::ifstream meanfile(param_.mean_value_file().c_str());
    Dtype mean, std;
    int i = 0;
    mean_values_.clear();
    scale_values_.clear();
    meanfile >> mean >> std;
    while (!meanfile.eof()) {
      mean_values_.push_back(mean);
      scale_values_.push_back(Dtype(1.)/std);
      LOG(ERROR) << "channel " << i << 
        " : mean " << mean_values_[i] << 
        " std " << 1. / scale_values_[i];
      ++i;
      meanfile >> mean >> std;
    }
  }
...
void DataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  const bool has_label = (label_datum != NULL) && (transformed_label != NULL);
  Blob<Dtype> buffer;
  buffer.Reshape(1, datum_channels, datum_height, datum_width);
  Dtype* buffer_data = buffer.mutable_cpu_data();
  Dtype* buffer_label = buffer.mutable_cpu_diff();
  if (rotate && phase_ == Caffe::TRAIN && has_uint8) {
    cv::Mat src, dst;
    src = cv::Mat::zeros(height, width, CV_8UC1); 
    cv::Point2f pt(src.cols/2., src.rows/2.);
    int angle = Rand(360);
    cv::Mat r = cv::getRotationMatrix2D(pt, angle, 1.0);
    for (int c = 0; c != datum_channels; ++c) {
      for (int data_index = 0; data_index != height * width ; ++data_index )  {
        Dtype datum_element =
          static_cast<Dtype>(static_cast<uint8_t>(data[data_index + c * datum_height * datum_width]));
    ...
    if (has_label)  {
      const string& label = label_datum->data();
      for (int data_index = 0; data_index != height * width ; ++data_index )  {
        Dtype datum_element =
          static_cast<Dtype>(static_cast<uint8_t>(label[data_index]));
        src.data[data_index] = datum_element;
      }
      cv::warpAffine(src, dst, r, cv::Size(src.cols, src.rows), cv::INTER_NEAREST);
      for (int data_index = 0; data_index != datum.height() * datum.width() ; ++data_index )  {
        buffer_label[data_index] = dst.data[data_index];
      }
      /*
      ...
      for (int data_index = 0; data_index != width * height; ++data_index)  {
        out.at<unsigned char>(data_index / width, data_index % width) = buffer_label[data_index] * 10;
      }
      cv::imwrite("out.tif",out);
      */
    }
  } else  {
      for (int data_index = 0; data_index != datum_channels * datum_width * datum_height; ++data_index )  {
      ...
      if (has_label)  {
        const string& label = label_datum->data();
        for (int data_index = 0; data_index != datum_width * datum_height; ++data_index )  {
          buffer_label[data_index] = static_cast<uint8_t>(label[data_index]);
        }
      }
  }
  ...
  for (int c = 0; c < datum_channels; ++c) {
    for (int h = 0; h < height; ++h) {
      for (int w = 0; w < width; ++w) {
        ...
        if (has_uint8) {
          datum_element = buffer_data[data_index];
        } else {
        ...
        if (has_mean_file) {
          ...
          if (has_mean_values) {
            datum_element -= mean_values_[c];
          }
          if (scale_values_.size()) {
            datum_element *= scale_values_[c];
          } else {
            datum_element *= scale;
          }
  ...
  if (has_label && this->param_.has_label_crop_size())  {
    unsigned int label_crop_size = this->param_.label_crop_size();
    int label_h_off = h_off + crop_size / 2 - label_crop_size / 2;
    int label_w_off = w_off + crop_size / 2 - label_crop_size / 2;
    // We only do random crop when we do training.
    width = this->param_.resize_label() > 0 ? this->param_.resize_label() : label_crop_size;
    height = this->param_.resize_label() > 0 ? this->param_.resize_label() : label_crop_size;
    int scale = width == 1 ? 0 : (label_crop_size -1) / (width - 1);
    for (int h = 0; h < height; ++h) {
      for (int w = 0; w < width; ++w) {
        data_index = (label_h_off + h * scale) * datum_width + label_w_off + w * scale;
        if (do_mirror) {
        ...
        if (has_uint8) {
          datum_element = buffer_label[data_index];
        } else {
          datum_element = label_datum->float_data(data_index);
        }
        transformed_label[top_index] = datum_element;
      }
    }
  ...
  if (this->param_.gitter_std() >= 0) {
    Dtype gitter_mean_range = this->param_.gitter_mean_range();
    Dtype gitter_mean;
    caffe_rng_uniform(1, -gitter_mean_range, gitter_mean_range, &gitter_mean);
    Dtype gitter_std = this->param_.gitter_std();
    Dtype* noise = buffer.mutable_cpu_diff();
    caffe_rng_gaussian(buffer.count(), gitter_mean, gitter_std, noise);
    for (int i = 0; i != buffer.count(); ++i) {
      transformed_data[i] += noise[i];
    }
  }
...
template<typename Dtype>
void DataTransformer<Dtype>::Transform(const Datum& datum,
                                       Blob<Dtype>* transformed_blob, Datum* label_datum, Blob<Dtype>* transformed_label) {
  ...
  if (transformed_label && label_datum) {
    Dtype* transformed_data_label = transformed_label->mutable_cpu_data();
    Transform(datum, transformed_data, label_datum, transformed_data_label);
  } else  {
