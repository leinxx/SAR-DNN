...
class L1LossLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  L1LossLayerTest()
    ...
    Dtype ignore_label = 1.2;
    for (int i = 0; i != blob_bottom_data_->count()/2; ++i) {
      blob_bottom_data_->mutable_cpu_data()[i] = ignore_label;
    }
  }
  ...
  void TestForward() {
    ...
    layer_param.mutable_loss_param()->add_ignore_label(1.2);
...
TYPED_TEST(L1LossLayerTest, TestForward) {
  this->TestForward();
}
