...
void DatumDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (target_lines_.size()) {
    ...
    top_shape[0] = batch_size;
    top_shape[1] = 1;
    top_shape[2] = label_crop_size > 0 ? label_crop_size : 1;
    top_shape[3] = label_crop_size > 0 ? label_crop_size : 1;
  } else {
    top_shape[0] = batch_size;
    top_shape[1] = 1;
    top_shape[2] = 1;
...
template <typename Dtype>
void DatumDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int idx = 0; idx != batch_size; ++idx) {
    ...
    if (target_lines_.size()) {
      ...
      this->transformed_label_.set_cpu_data(prefetch_label + batch->label_.offset(idx));
    ...
    offset = batch->label_.offset(idx);
    for (int i = 0; i != batch->label_.count(1); ++i)  {
      if (nclass) {
        for (int j = 0; j != binary_labels_.size(); ++j)  {
          if (prefetch_label[offset + i] == binary_labels_[j])  {
  ...
  //write_blob_to_image(batch->data_, batch->label_);
  //write_blob_to_image(batch->data_, false);
  //write_blob_to_image(batch->label_, true);
}
      ...
      for (int i = 0; i != H; ++i)  {
        for (int j = 0; j != W; ++j, ++offset)  {
          if (!is_label)  {
            ...
            mat.at<unsigned char>(i,j) = blob.cpu_data()[offset] * 10;
          }
        }
      }
...
REGISTER_LAYER_CLASS(DatumData);
