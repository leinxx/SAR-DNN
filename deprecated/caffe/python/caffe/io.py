...
def arraylist_to_datumvector_str(arraylist, labellist):
  """Converts a list of arrays to a serialized blobprotovec, which could be
  then passed to a network for processing.
  """
  vec = caffe_pb2.DatumVector()
  vec.datums.extend([array_to_datum(arr, label) for arr, label in zip(arraylist, labellist)])
  return vec.SerializeToString()
...
def datumvector_str_to_arraylist(str):
  """Converts a list of datumn to numpy array
  """
  vec = caffe_pb2.DatumVector()
  vec.ParseFromString(str)
  return [[datum_to_array(datum), datum.label] for datum in vec.datums]
