...
template <typename Dtype>
void fill_predict(cv::Mat& predict, const Blob<Dtype>* src, const vector<cv::Point> locs, const float scale const int min_val, const int max_val) {
...
// n : number of src data to fill
  for (int i = 0; i != n; ++i) {
    ...
    v = v > 100 ? 100 : v;
    v = v < 0 ? 0 : v;
    predict.at<unsigned char>(r, c) = v;
  }
}
...
int main(int argc, char** argv) {
  ...
  CHEK_EQ(feature_c, 1) << "only one channels is supproted due to the implementation here is using opencv";
  for (int r = 0; r != H-height+1; r += feature_h) {
    for (int c = 0; c != W-width+1; c += feature_w)  {
      // extract patch
      if (have_mask == false || mask.at<unsigned char>(r + height / 2, c + width / 2) == 0) {
        ...
        locs[n] = cv::Point(c + width / 2 - feature_w / 2, r + height / 2 - feature_h / 2); // assuming the prediction has the same sampling rate as the data
        ++n;
      }
      if (n == batch_size)  {
        // predict
        ...
        fill_predict(predict, feature_blob.get(), locs, FLAGS_scale, FLAGS_min, FLAGS_max);
