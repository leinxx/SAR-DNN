...
template <typename Dtype>
class DatumDataLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit DatumDataLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  virtual ~DatumDataLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "DatumData"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  virtual inline int ExactNumTopBlobs() const { return 2; }
 ...
 protected:
  ...
  vector<std::string> lines_, target_lines_;
  vector<int> lines_idx_, prefetch_idx_;
  int lines_id_;
  vector<Datum> prefetch_buffer_, prefetch_target_buffer_;
  int current_idx_;
  bool shuffle_;
